...
addelectric(float* __restrict__ Bx, float* __restrict__ By, float* __restrict__ Bz,
            ...
            float* __restrict__ eLUT2d, float* __restrict__ pLUT2d,
    ...
    float e__; // electric field
    float p__; // polarization
    ...
    e__ = eLUT2d[symidx(r0, regions[i_])];
    p__ = pLUT2d[symidx(r0, regions[i_])];
    B.x -= (e__*p__/cx) * m_.z;
    B.z += (e__*p__/cx) * m_.x;
